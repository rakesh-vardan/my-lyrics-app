...
export default async function MoviesPage() {
  ...
  return (
    <div>
      ...
      <h1 className="text-2xl font-bold mb-6 text-violet-400">🎬 Browse by Movie</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {sortedMovies.map(([movie, { count, songs: movieSongs }]) => (
          <div
            key={movie}
            className="bg-slate-800 border border-slate-700 rounded-xl p-4 hover:border-violet-500 transition-colors"
          >
            ...
            <div className="space-y-1">
              {movieSongs.map((s) => (
                <Link
                  key={s.id}
                  href={`/song/${s.id}`}
                  className="block text-sm text-slate-400 hover:text-violet-400 transition-colors truncate telugu-text"
                >
                  {s.title_telugu}
                  {s.title_english && (
